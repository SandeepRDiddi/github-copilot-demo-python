...
def create_server(app_settings: Settings | None = None) -> FastMCP:
    resolved_settings = app_settings or settings
    context = AppContext.create(settings=resolved_settings)
    ...
    def server_status() -> dict[str, object]:
        ...
        return {
            ...
            "transport": context.settings.transport,
            "started_at": context.started_at.isoformat(),
            "workspace_root": context.settings.workspace_root.as_posix(),
    ...
    def server_health() -> dict[str, object]:
        """Return a lightweight runtime status summary for operational checks."""
        return {
            "status": "ok",
            "environment": context.settings.environment,
